...
<section class="section">
    ...
    <h1>Absensi @if (isset($kelas))kelas  <span class=" text-danger">{{$kelas->nama}}</span> Semester <span class=" text-danger">{{$filter['id_semester']}} </span>@endif </h1>
    </div>
    <div class="section-body">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <form class="row" action="{{route('absensi.murid')}}" method="get">
                        @csrf
                        <div class="col-sm-3">
                            ...
                            <table class="table table-bordered table-md">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$key+1}}</td>
                                        <td>{{$murid->nama}}</td>
                                        @forelse ($murid->absensi as $absensi)
                                            <td><input type="text" name="sakit[]" value="{{$absensi->sakit}}"></td>
                                            <td><input type="text" name="ijin[]" value="{{$absensi->ijin}}"></td>
                                            <td><input type="text" name="alpa[]" value="{{$absensi->alpa}}"></td>
                                            @empty
                                            <td><input type="text" name="sakit[]" ></td>
                                            <td><input type="text" name="ijin[]" ></td>
                                            <td><input type="text" name="alpa[]" ></td>
                                        @endforelse
                                    </tr>
                                    @endforeach
                                    @endif
